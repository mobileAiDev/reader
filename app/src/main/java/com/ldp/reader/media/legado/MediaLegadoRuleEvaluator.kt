...
class MediaLegadoRuleEvaluator(
    private val urlBuilder: MediaLegadoUrlBuilder = MediaLegadoUrlBuilder(),
    private val jsonPath: MediaJsonPathLite = MediaJsonPathLite()
) {
    ...
    fun list(rule: String?, context: BodyContext): List<RuleNode> {
        ...
        splitCombinedCandidates(cleanRule)
            .takeIf { it.size > 1 }
            ?.let { candidates ->
                return candidates.flatMap { candidate -> list(candidate, context) }
            }
    ...
    private fun splitCombinedCandidates(rule: String): List<String> {
        val candidates = ArrayList<String>()
        var templateDepth = 0
        var start = 0
        var index = 0
