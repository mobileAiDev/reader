...
data class V8ChapterInput(
    val index: Int,
    val title: String,
    val content: String,
    val contentQualitySignal: V8ContentQualitySignal? = null
)

data class V8ContentQualitySignal(
...
enum class V8ChapterQualityType {
    CLEAN_STORY,
    CLEAN_WITH_TRIM,
    NON_STORY,
    BAD_EXTRACTION,
...
data class V8ChapterQualityResult(
    val chapterIndex: Int,
    val chapterTitle: String,
    val type: V8ChapterQualityType,
    val cleanText: String,
    val confidence: Double,
    val reasons: List<String>,
    val metrics: V8QualityMetrics
) {
    val usableForStory: Boolean
        get() = type == V8ChapterQualityType.CLEAN_STORY || type == V8ChapterQualityType.CLEAN_WITH_TRIM
}

class V8ChapterQualityGate {
    fun inspect(chapter: V8ChapterInput): V8ChapterQualityResult {
        ...
        if (metrics.metaLines > 0 && type == V8ChapterQualityType.NON_STORY) {
            reasons.add("non-story meta signals=${metrics.metaLines}")
        }

        return V8ChapterQualityResult(
    ...
    private fun classifyChapter(
        ...
        if (!numberedTitle && strongTitleMeta && (bodyMetaHits >= 1 || !hasStoryShape || metrics.cleanedChars <= 1_200)) {
            ...
            return V8ChapterQualityType.NON_STORY
        }
        ...
        if (metrics.cleanedChars < 120) {
            ...
            return if (metrics.shellLines > 0) V8ChapterQualityType.BAD_EXTRACTION else V8ChapterQualityType.TOO_SHORT_UNCERTAIN
        }
        if (!hasStoryShape) {
            reasons.add("clean text lacks stable narrative shape")
            return V8ChapterQualityType.MIXED_EXTRACTION_UNCERTAIN
        }
        return if (metrics.removedChars > 0 || metrics.removedLines > 0) {
            V8ChapterQualityType.CLEAN_WITH_TRIM
        } else {
    ...
    private fun isNarrativeLine(line: String): Boolean {
        val chineseChars = line.count { char -> char in '\u4e00'..'\u9fff' }
        val punctuation = line.count { char -> char == '。' || char == '！' || char == '？' || char == '”' || char == '；' }
    ...
    private fun confidenceFor(type: V8ChapterQualityType, metrics: V8QualityMetrics): Double {
        return when (type) {
            V8ChapterQualityType.CLEAN_STORY -> 0.95
            V8ChapterQualityType.CLEAN_WITH_TRIM -> (0.82 + metrics.removedCharRatio.coerceAtMost(0.12)).coerceAtMost(0.92)
            V8ChapterQualityType.NON_STORY -> 0.88
            V8ChapterQualityType.BAD_EXTRACTION -> (0.80 + metrics.shellLineRatio.coerceAtMost(0.18)).coerceAtMost(0.98)
            V8ChapterQualityType.TOO_SHORT_UNCERTAIN -> 0.58
            V8ChapterQualityType.MIXED_EXTRACTION_UNCERTAIN -> 0.62
        }
    }
    ...
    companion object {
        ...
        private val strongNonStoryTitlePatterns = listOf(
            Regex("""(?:请假|请假条|休息一天|停更|更新说明|更新调整|通知|公告|作者的话|作者说|上架感言|完本感言|完结感言|新书|推书|推一本书|推荐一本书|番外说明|中奖|抽奖|求票|求月票|致谢|写在最后|后记|完结|写给书友|书友的一封信|茶话会)""")
        )
