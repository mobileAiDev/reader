...
class NovelPollutionAnalyzer(
    ...
    fun analyze(
        ...
        retainChunkScores: Boolean = true,
        ...
        val finalDetection = scoreAndDetectFinal(chunks, fingerprint, retainChunkScores, progress)
        val detection = finalDetection.detection
        ...
        val reportChunkScores = finalDetection.reportChunkScores
    ...
    private fun scoreAndDetectFinal(
        ...
        return FinalDetectionResult(
            detection = detection,
            reportChunkScores = if (retainChunkScores) scoredChunks else emptyList()
        )
    }
    ...
    private data class SameBookArcEvidence(
        ...
        fun absorbs(scores: StructuralScores): Boolean {
            ...
            val nearbyStrongBridgeArc = nearbyPastChapters.isNotEmpty() &&
                nearbyPastBridgeScore >= 0.85 &&
            ...
            val repeatedPastArc = pastMatchedChapters.size >= 2 &&
                ...
                supportScore >= 0.42
            val nearbyFutureBridgeArc = nearbyFutureChapters.isNotEmpty() &&
                maxBridgeScore >= 0.85 &&
                scores.futureIntegration >= 0.75 &&
                scores.prefixBookStrength >= 0.55 &&
                scores.worldConsistency >= 0.90 &&
                scores.graphAbsorption >= 0.25 &&
                scores.prototypeSimilarity >= 0.08
    ...
    private data class FinalDetectionResult(
        val detection: StructuralDetectionOutput,
